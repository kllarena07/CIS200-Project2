...
int main() {
  ...
  for (size_t i = 0; i < MAX_JOBS; ++i) {
    ...
    new_job.job_type = 'B';
    new_job.arrival_time = i + get_random_int(2, 8);
    new_job.processing_time = get_random_int(3, 13);

    insert_node(head, new_job);

    new_job.job_type = 'C';
    new_job.arrival_time = i + get_random_int(4, 18);
    new_job.processing_time = get_random_int(6, 12);

    insert_node(head, new_job);

    new_job.job_type = 'D';
    new_job.arrival_time = i + get_random_int(2, 12);
    new_job.processing_time = get_random_int(2, 22);

    insert_node(head, new_job);
  }

  // Overwriting all the data purposely
  ...
  while (current != nullptr) {
    ...
    // Uncomment the above code if you want to write this data to a .txt file
    // and comment the .write() below
